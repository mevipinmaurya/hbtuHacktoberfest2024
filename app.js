...
// Create stars for the celestial background
...
document.getElementById('importTasks').addEventListener('click', () => document.getElementById('importInput').click());
...
function exportTasks() {
    ...
    openmodal('green', 'Tasks exported successfully!');
}
...
function importTasks(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            const existingTasks = getItemFromLocalStorage();
            const mergedTasks = [...existingTasks];

            importedTasks.forEach(task => {
                if (!existingTasks.some(existing => existing.task === task.task)) {
            ...
            location.reload();
            openmodal('green', 'Tasks imported successfully!');
        } catch (err) {
            openmodal('red', 'Error importing tasks. Please check the file format.');
        }
    };
